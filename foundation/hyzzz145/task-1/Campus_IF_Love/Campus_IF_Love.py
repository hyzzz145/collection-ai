...
class Character:
    ...
    def talk(self):
        ...
        pause(0.5)
        print(f"\n{self.role}：{DIALOGUES[self.name][turn]['text']}")
        optionA = DIALOGUES[self.name][turn]['optionA']
        optionB = DIALOGUES[self.name][turn]['optionB']
        pause(0.5)
        print(f"你回答到 1：{optionA}\n          2：{optionB}")
        pause(0.2)
        choice = input("此时你选择：（1 or 2）")
        #对话的好感度增加设置
        pause(0.5)
        if choice == '1':
            print(f"你说：{optionA}")
    ...
    def give_gift(self, gift):
        ...
        value = GIFT_EFFECTS[gift][self.name]
        pause(0.5)
        if value >= 10:
            print(f"{self.name}激动地收下了{gift}。")
        elif value > 0:
            print(f"{self.name}开心地收下了{gift}。")
        else:
            print(f"{self.name}疑惑地收下了{gift}。")
...
class Game:
    ...
    def scene_xiaobai(self):
        ...
        if choice == '1':
            pause(0.2)
            print("\n你轻拿过小白手下压着的思路纸，在纸上加了几段精妙的算法思路，引得小白眼前一亮。")
            pause(0.5)
            print("小白微微一笑，放下了紧绷的神经，对你十分敬佩。你进入了【小白线】！")
    ...
    def scene_jiejie(self):
        ...
        if choice == '1':
            pause(0.2)
            print("\n你微笑着点头，主动帮她拉开椅子，并将手里温热的奶茶轻轻推到桌中央。")
            pause(0.5)
            print("姐姐眼中闪过一丝讶异与欣赏，嘴角扬起温柔的弧度。你进入了【姐姐线】！")
            self.current_target = self.characters["姐姐"]
            self.story_loop()
            return True
        else:
            pause(0.5)
            print("姐姐微微皱了皱眉，缓缓的离开了，你错过了一段长谈的缘分。")
            return False
        # TODO 两种选择 如果选择了1 则进入该位角色的故事线 并返回 True 如果选择了 2 则进入下一位角色的选择 并且返回False
        #要求同上1
    ...
    #定义对话函数
    '''
    def talk(self,target_charactor:Character,text_turn:int) ->  None:
        print(f"\n{DIALOGUES[self.name][text_turn]['text']}")
        optionA = DIALOGUES[self.name][text_turn]['optionA']
        optionB = DIALOGUES[self.name][text_turn]['optionB']
        print(f"你回答到 1：{optionA}；2：{optionB}")
        choice = input("此时你选择：（1 or 2）")
        if choice == '1':
            print(f"你说：{optionA}")
    '''
...
GIFT_EFFECTS = {
    ...
    "奇怪的石头": {"学姐": -10, "小白": -10, "姐姐": -10},  # 所有人 -10
